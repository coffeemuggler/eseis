  ...
  if (*ppmstl)
  {
    ...
    while (id)
    {
      ...
      while (seg)
      {
        ...
        /* REC - free samprate list if allocated */
        MSSampRateList *sr_ptr, *sr_free;
        sr_ptr = sr_free = seg->samprate_list;
        while (sr_ptr != NULL) {
                sr_ptr = sr_ptr->next;
                free (sr_free);
                sr_free = sr_ptr;
...
mstl_msr2seg (MSRecord *msr, hptime_t endtime)
{
  ...
  /* REC -- adding on a samprate tracking structure so we can get the
   * mode (or most common) sample rate to represent as the definitive
   * segment sample rate
   */
...
mstl_addmsrtoseg (MSTraceSeg *seg, MSRecord *msr, hptime_t endtime, flag whence)
{
  ...
  /* REC -- adding to a samprate tracking structure so we can get the
   * mode (or most common) sample rate to represent as the definitive
   * segment sample rate.  Use the msr sample rate as seg only recorded
                     ...
                     flag details, flag gaps)
{
  ...
  while (id)
  {
    ...
    while (seg)
    {
      /* REC -- use the mode of segment sample rate */
      seg->samprate = get_segsamprate_mode(seg->samprate_list);
...
mstl_printsynclist (MSTraceList *mstl, char *dccid, flag subsecond)
{
  ...
  while (id)
  {
    ...
    while (seg)
    {
      ...
      /* REC -- print mode of segment sample rate */
      seg->samprate = get_segsamprate_mode(seg->samprate_list);
 ...
 * count to an existing node with the same value.  The result is a histogram of sample rates.
 *
 * Return - pointer to head of linked list.
 */
MSSampRateList* add_segsamprate(MSSampRateList *srl, double samprate) {
        MSSampRateList *head, *current, *prev = NULL;
        head = current = srl;
        /*printf("DEBUG: add_segsamprate %f\n",samprate);*/
        while (current != NULL) {
                /* find a node with our sample rate */
                /*printf("DEBUG: -- add_segsamprate compare: %f to %f\n",current->samprate,samprate);*/
                if (current->samprate == samprate) {
                        current->count++;
                        /*printf("DEBUG: ---- add_segsamprate +1: %f,%d\n",current->samprate,current->count);*/
                        break;
                } else {
                        prev = current;
        ...
        if (current == NULL) {
                /* create new node with a count of 1 */
                current = (MSSampRateList*) malloc(sizeof(MSSampRateList));
                current->samprate = samprate;
                current->count = 1;
                /*printf("DEBUG: ---- add_segsamprate new: %f,%d\n",current->samprate,current->count);*/
                current->next = NULL;
                if (prev != NULL) prev->next = current;  /* add to linked list */
                else head = current;
        }
        return head;

} /* End of add_segsamprate */

/* REC - non-standard function addition.  To the indicated pointer to an associative linked list
 * of sample rate counts, return the sample rate with the highest count value, or 0.0 if the
 * parameter was null.
 */
double get_segsamprate_mode(MSSampRateList *srl) {
        MSSampRateList *current,*max;
        current = max = srl;
        /*printf("DEBUG: get_segsamprate_mode\n");*/
        /* loop over the sample rate list and find the max count */
        while (current != NULL) {
                /* find a node with our sample rate */
                if (max == NULL || current->count > max->count)
                        max = current;
                /*printf("DEBUG: ---- get_segsamprate_mode max: %f\n",max->samprate);*/
                current = current->next;
        }
