...
#' case provide a value for \code{"window"}, which is the smoothing 
...
#' 
#' @param window \code{Numeric} value, window size (number of samples) of 
...
#' plot(hv, type = "l")
#' 
#' ## calculate h-v-ratio with a smmothing window equivalent to dt
#' hv <- signal_hvratio(data = s, 
#'                      dt = 1 / 200,
#'                      window = 200)
#' plot(hv, type = "l")
...
signal_hvratio <- function(
  ...
  if(missing(window) == FALSE) {
    ...
    ## apply running mean
    s_log_smooth <- lapply(X = s_log, FUN = function(X, window) {
